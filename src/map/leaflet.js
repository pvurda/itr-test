(function(global){
  ...
  function drawIsochrones(options){
    const L = options.L;
  ...
  function drawEventRings(options){
    const L = options.L;
    const layers = options.layers;
    const ev = options.ev;
    ...
    for (const t of ticks){
      const r = Math.max(0, v * t / det);
      const isPast = t <= dtAnchorSec;
      L.circle([lat0, lon0], {radius: r, color: "#555", weight: 1, fill: false, opacity: isPast ? 0.16 : 0.22, interactive: false}).addTo(layers.eventCircleLayer);
      const p = destinationPoint(lat0, lon0, r, 90);
      const lab = (t < 60) ? `${t}s` : `+${Math.round(t / 60)}m`;
      const icon = L.divIcon({className: "", html: `<div class="circle-label">${lab}</div>`, iconSize: [0, 0]});
      L.marker([p.lat, p.lon], {icon, interactive: false}).addTo(layers.eventLabelLayer);
    }

    const t0Str = fmtHHMMSS(new Date(ev.t0ms));
    const aTag = (anchor && anchor.source === "det" && anchor.camCode) ? `A: ${anchor.camCode}` : "A: EVENT";
    const metaText = `T0: ${t0Str} · Δt: +${fmtTimeSeconds(dtEventSec)} · ${aTag} ${anchorTime} · ΔA: +${fmtTimeSeconds(dtAnchorSec)} · Rnow: ${Math.round(rNow)} m`;

    return {metaText, rNow};
  }

  function setPulse(options){
    const L = options.L;
    const layers = options.layers;
